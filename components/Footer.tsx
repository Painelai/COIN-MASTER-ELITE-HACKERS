...
export const Footer: React.FC = () => {
  const { content, locale, config } = useCore();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8 mt-12">
            ...
            <div className="flex flex-wrap justify-center md:justify-end gap-4 md:gap-8 text-xs text-secondary font-medium uppercase tracking-wide">
                ...
                <a href="#" onClick={(e) => openLegalModal(t.footer_terms, e)} className="hover:text-brand-400 transition-colors whitespace-nowrap">
                   {t.footer_terms}
                </a>
                <a href="#" onClick={(e) => openLegalModal(t.footer_refund, e)} className="hover:text-brand-400 transition-colors whitespace-nowrap">
                   {t.footer_refund}
                </a>
                <a href="#" onClick={(e) => openLegalModal(t.footer_privacy, e)} className="hover:text-brand-400 transition-colors whitespace-nowrap">
                   {t.footer_privacy}
                </a>
            </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
          ...
          <div className="text-muted text-[10px] uppercase font-bold tracking-widest whitespace-nowrap">
            {t.footer_prototype_label} v{config.version}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-surface border border-border-highlight w-full max-w-md rounded-xl p-6 shadow-2xl animate-fade-in">
             ...
             <div className="space-y-4 text-secondary text-sm h-64 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-surface-highlight">
               <p>{t.footer_modal_demo_text}</p>
               <p className="text-xs text-muted mt-4">v{config.version}</p>
             </div>
