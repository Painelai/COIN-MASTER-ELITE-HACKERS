    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                // VÍNCULO DINÂMICO: Usa o template definido no banco para novos membros
                const shouldAddRandomUser = Math.random() < 0.12;
                let randomUserMsg: Message | null = null;
                
                if (shouldAddRandomUser) {
                    const randomName = generateUniqueIdentity();
                    const template = content.socialProof.userAddedTemplate || "{name} foi adicionado";
                    randomUserMsg = {
                        id: `sys-add-${Date.now()}`,
                        user: "Sistema",
                        text: template.replace('{name}', randomName), 
                        isUser: false,
                        isSystem: true,
                        isUserAdded: true,
                        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        ...
        return () => clearTimeout(timer);
    }, [timelineIndex, activeRoom, bots, staticMode, isOpen, messages, content.socialProof]);
    ...
    return (
        <>
            {restrictedModal.open && (
                <div className="fixed inset-0 z-[8000] flex items-center justify-center p-4 animate-fade-in">
            ...
            <button onClick={() => setIsOpen(!isOpen)} className={`fixed z-[48] bottom-4 right-4 md:right-6 w-14 h-14 bg-brand-600 hover:bg-brand-500 text-white rounded-full shadow-neon flex items-center justify-center transition-all hover:scale-110 active:scale-95 ${!isOpen && 'animate-bounce-slow'}`}>
            ...
            <div className={`fixed ${hasBottomOffset ? 'bottom-4 md:bottom-24' : 'bottom-4 md:bottom-6'} right-4 md:right-6 w-[calc(100vw-32px)] md:w-[380px] h-[600px] max-h-[75vh] md:max-h-[80vh] bg-[#e5ddd5] border border-border-dim rounded-2xl shadow-2xl z-[47] flex flex-col overflow-hidden transition-all duration-300 ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none translate-y-10'}`}>
                ...
                <div className="bg-[#075e54] text-white p-4 flex items-center justify-between shadow-md z-10 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="cursor-pointer flex flex-col items-start" onClick={() => setShowParticipants(!showParticipants)}>
                            <h3 className="text-sm font-black uppercase tracking-widest leading-none truncate max-w-[150px] md:max-w-none">{activeRoom?.name || 'Elite VIP Group'}</h3>
                        ...
                        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                                    ...
                                    >
                                        {msg.isSystem || msg.isUserAdded ? (
                                            <div className={`${theme.bubble} flex items-center gap-3 animate-slide-up-fade`}>
                                                ...
                                                <span className={theme.text}>{msg.text}</span>
                                            </div>
                                        ) : (
                                            <div className={`flex items-center gap-2 max-w-[90%] md:max-w-[85%] ${msg.isUser ? 'flex-row' : 'flex-row-reverse'}`}>
                                                {!msg.isSystem && !msg.isGoldReview && !msg.isUser && (
                                                    <button onClick={(e) => { e.stopPropagation(); setReplyingTo(msg); }} className="p-1.5 bg-white/20 hover:bg-white/40 rounded-full opacity-0 group-hover:opacity-100 text-slate-500 transition-all shrink-0">
                                                        <CornerUpLeft size={16} strokeWidth={2.5} />
                                                    </button>
                                                )}
                                                <div className="relative min-w-0">
                                                    <div className={`relative px-3 py-2 rounded-2xl shadow-sm transition-all active:scale-[0.98] ${theme.bubble} ${theme.text} ${msg.isUser ? 'rounded-tr-none' : 'rounded-tl-none'} ${isSelected ? 'ring-2 ring-emerald-500' : ''}`}>
                                                        {theme.tail !== 'hidden' && <div className={`absolute top-0 w-3 h-4 ${theme.tail} ${msg.isUser ? '-right-2 clip-tail-right' : '-left-2 clip-tail-left'}`}></div>}
                                                        {!msg.isSystem && !msg.isUser && !msg.isGoldReview && <p className="text-[10px] font-black uppercase leading-none text-blue-600 mb-1">{msg.user}</p>}
                                                        {msg.replyTo && !msg.hideQuote && (
                                                            <div className="mb-2 p-2 bg-black/5 border-l-4 border-emerald-500 rounded text-[10px] opacity-70 overflow-hidden">
                            ...
                            {typingUser && (
                                <div className="flex flex-col items-start gap-1 ml-2 mb-4 animate-fade-in">
                                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter ml-1">{typingUser} está digitando...</span>
                        ...
                        <div className="p-2 bg-[#f0f0f0] border-t border-slate-300 shrink-0">
                            {replyingTo && (
                                <div className="p-2 bg-white border-l-4 border-emerald-500 rounded-lg mb-2 flex justify-between items-center animate-slide-up">
                                    <div className="min-w-0"><p className="text-[10px] font-black text-emerald-600">Respondendo a {replyingTo.user}</p><p className="text-xs text-slate-500 truncate">{replyingTo.text}</p></div>
                            ...
                            <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="flex items-center gap-2">
                                <div className="flex-1 bg-white rounded-full flex items-center px-3 py-1 shadow-sm border border-slate-200">
                                    <Smile ref={emojiButtonRef} className="w-6 h-6 text-slate-400 cursor-pointer mr-2" onClick={() => setShowEmojiPicker(!showEmojiPicker)} />
                                ...
                                <button type="submit" className="w-11 h-11 bg-[#075e54] text-white rounded-full flex items-center justify-center shadow-lg transition-all shrink-0"><Send className="w-5 h-5 ml-1" /></button>
                            </form>
                        </div>
                    </div>
                            ...
                            {participantsTab === 'membros' ? (
                                <div className="space-y-4">
                                    {participantBots.length > 0 ? participantBots.map(bot => {
                                        ...
                                        return (
                                            <div key={bot?.id} className="flex items-center gap-4 border-b border-slate-100 pb-3 last:border-0 group/member">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-[9px] text-slate-500 uppercase font-black tracking-tight flex items-center gap-1.5"><Clock size={10} className="text-emerald-500" /> Usou {status.tool} há {status.min} min</p>
                                                </div>
                                                <div className="flex items-center gap-2 shrink-0">
                                                    <div className="flex items-center gap-1.5 bg-brand-500/10 px-2 py-1 rounded-lg border border-brand-500/20"><span className="text-[8px] font-black text-brand-600 uppercase">Membro Elite</span><Shield size={12} className="text-brand-600" /></div>
                                                    <button onClick={(e) => { e.stopPropagation(); handleEliteRestriction('adicionar um amigo'); }} className="p-2 text-slate-400 hover:text-emerald-500 transition-all"><UserPlus size={18} /></button>
                                                </div>
                                            </div>
                                        );
                                    }) : <div className="py-10 text-center text-slate-400 text-xs font-bold uppercase opacity-50">Sincronizando...</div>}
                                </div>
                            ) : (
                                <div className="space-y-4">
                                    {[
                                        ...
                                        <div key={i} className="bg-white p-4 rounded-xl border border-slate-200 flex items-center gap-4 group hover:border-brand-500/50 transition-all cursor-pointer" onClick={() => handleEliteRestriction('acessar grupos exclusivos')}>
                                            ...
                                            <div className="flex-1"><h5 className="text-sm font-bold text-black">{g.name}</h5><p className="text-[10px] text-slate-400 font-bold uppercase">Restrito: Licença Vitalícia</p></div>
